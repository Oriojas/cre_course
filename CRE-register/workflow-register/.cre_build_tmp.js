...
var network282;
var evmClient;
function onEn5Munutos(runtime2, payload) {
  if (!payload.scheduledExecutionTime)
    throw new Error("Missing scheduledExecutionTime");
  runtime2.log("En 5 munutos triggered at " + payload.scheduledExecutionTime);
  const reportPayload = encodeAbiParameters(parseAbiParameters("string myInfo"), ["Oscar, Colombia"]);
...
function initWorkflow(config) {
  ...
  return [
    cre.handler(cronTrigger.trigger({ schedule: config.schedule }), onEn5Munutos)
  ];
}
async function main() {
